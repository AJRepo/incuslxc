...
addition, recorded in the {ref}`Incus database <database>`.
...
If you use only the `default` profile, or only the standard `incusbr0` network bridge,
...
back up the `/var/lib/incus` directory.
...
1. Stop Incus on your server (for example, with
...
1. Restart Incus (for example, with
   `sudo systemctl start incus.socket incus.service` or by restarting your machine).
...
its instance volume, see {ref}`storage-volume-types`).
...
{ref}`Incus database <database>`, it can still be very convenient
